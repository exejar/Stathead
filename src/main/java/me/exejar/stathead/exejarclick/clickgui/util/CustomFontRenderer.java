...
public class CustomFontRenderer {
    ...
    //  All credit for this CustomFontRenderer goes to the creator of Exhibition, Arithmo. (At least that's who I think created this...)
    ...
    static {
        CustomFontRenderer.RANDOM_OFFSET = 1;
    }

    public CustomFontRenderer(final Font font, final boolean antialias) {
    ...
    public CustomFontRenderer(final Font font, final int characterCount, final boolean fractionalMetrics, final boolean antialias) {
        this.fractionalMetrics = false;
        this.colorCodes = new int[32];
        this.font = font;
        this.fractionalMetrics = fractionalMetrics;
        this.regularData = this.setup(new CharacterData[characterCount], 0, antialias);
        this.boldData = this.setup(new CharacterData[characterCount], 1, antialias);
        this.italicsData = this.setup(new CharacterData[characterCount], 2, antialias);
    }

    private CharacterData[] setup(final CharacterData[] characterData, final int type, final boolean antialias) {
        this.generateColors();
    ...
    public void drawStringWithShadow(final String text, final float x, final float y, final int color) {
        ...
        this.renderString(text, x, y, Colors.getColor(0, color >> 24 & 0xFF), true);
        GL11.glTranslated(-0.5, -0.5, 0.0);
        this.renderString(text, x, y, color, false);
    }
                    ...
                    if (character == '\247' && i < length) {
                        ...
                        if (index >= 16) {
                            if (index == 16) {
                                ...
                                GL11.glColor4d(1.0 * (shadow ? 0.25 : 1.0), 1.0 * (shadow ? 0.25 : 1.0), 1.0 * (shadow ? 0.25 : 1.0), 1.0);
                            }
                        }
                        else {
                            obfuscated = false;
                        ...
                        if (obfuscated) {
                            character += (char)CustomFontRenderer.RANDOM_OFFSET;
                        }
